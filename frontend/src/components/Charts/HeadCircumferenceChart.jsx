// components/Charts/HeadCircumferenceChart.jsx
...
const HeadCircumferenceChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const headPercentiles = response.data.percentiles.head;
        
        // Crear un conjunto de todas las edades únicas
        const allAges = new Set();
        measurements.forEach(m => allAges.add(m.age_weeks));
        headPercentiles.forEach(p => allAges.add(p.age_weeks));
        ...
        const combinedData = sortedAges.map(age => {
          const headCircumference = findByAge(measurements, age, 'head_circumference');
          const p3 = findPercentile(headPercentiles, age, 3);
          const p50 = findPercentile(headPercentiles, age, 50);
          const p97 = findPercentile(headPercentiles, age, 97);
  ...
  return (
    <Card>
      <Card.Header>
        <h3>Gráfico de Perímetro Craneal - {baby?.name}</h3>
      </Card.Header>
      <Card.Body>
        <ResponsiveContainer width="100%" height={400}>
          ...
          >
            ...
            />
            <YAxis 
              label={{ value: 'Perímetro Craneal (cm)', angle: -90, position: 'insideLeft' }} 
            />
            <Tooltip />
            <Legend />
            ...
            <Line 
              ...
              strokeWidth={2} 
              activeDot={{ r: 8 }} 
              name="Perímetro craneal del bebé" 
            />
          </LineChart>
        </ResponsiveContainer>
        ...
        <div className="mt-3">
          <p>
            <strong>Interpretación:</strong> Este gráfico muestra el perímetro craneal de {baby?.name} en comparación con los 
            percentiles estándar de la OMS. La línea naranja representa el perímetro craneal real del bebé, mientras que las 
            áreas sombreadas muestran el rango normal (entre los percentiles 3 y 97).
          </p>
          <p>
            El perímetro craneal es un indicador importante del desarrollo cerebral. Un crecimiento constante dentro 
            de los percentiles normales es generalmente un buen signo. Cualquier cambio brusco o medidas fuera de los 
            percentiles normales deben ser consultados con el pediatra.
          </p>
        </div>
      </Card.Body>
    </Card>
